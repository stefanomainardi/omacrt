...
pub enum Kind {
    ...
    /// A file to delete.
    Stale(PathBuf),
    /// The desktop plugins, older than the version that is running: they
    /// are copies in the user's own configuration, and a package upgrade
    /// cannot reach them. Clearing it means writing this version's files.
    OldPlugin(usize),
}
...
pub fn survey() -> Vec<Mess> {
    ...
    out.extend(old_plugin());
    out
}

/// The desktop plugins, when they are not the ones this version carries.
///
/// They are copies in the user's own configuration: Omarchy loads a plugin
/// from a folder, and its validator refuses a symlink inside one. So an
/// upgrade of the program leaves the widget, the panel and the overlay as
/// they were, running against a binary that has moved on. A package manager
/// cannot fix that: it does not write into anybody's home.
fn old_plugin() -> Vec<Mess> {
    if !crate::plugin::installed() {
        return Vec::new();
    }
    let drift = crate::plugin::drift();
    if drift.is_empty() {
        return Vec::new();
    }
    let names: Vec<String> = drift
        ...
        .collect();
    vec![Mess {
        what: "old plugin".into(),
        detail: format!(
            "the desktop plugin is not this version: {}",
            names.join(", ")
        ),
        fix: "write this version's files".into(),
        kind: Kind::OldPlugin(drift.len()),
    }]
}
...
pub fn clear(mess: &Mess) -> (bool, String) {
    match &mess.kind {
        ...
        Kind::OldPlugin(_) => match crate::plugin::sync(false) {
            Ok(wrote) if wrote.is_empty() => (true, "already this version".into()),
            Ok(wrote) => (true, format!("{} file(s) written", wrote.len())),
            Err(e) => (false, e),
        },
    }
}
