...
facts:
  what it is: >
    A Wayland compositor. One process, one thread, one event loop, 3616 lines
    across four files, on smithay 0.7.
  what it drives: >
    One DRM connector taken from the desktop by lease and programmed with a
    15 kHz modeline: a television, not a monitor. The desktop keeps running on
    its other outputs.
  status: >
    Experimental. It drives hardware that a wrong signal destroys, and the
    guard that refuses one is in part 10.
  where: >
    Part of OmaCRT, MIT licensed, at github.com/stefanomainardi/omacrt. The
    code and the measuring instrument are both in the repository.
conditions:
  ...
  kernel: Arch linux 7.2.4.arch1-2, unpatched, and no amdgpu option on the command line
...
<p align="center">
  <img src="../media/architecture.svg" width="976" alt="How Flyback is put together: Hyprland leases the connector to it, the clients reach it over a private Wayland socket, and it drives amdgpu, the converter and the television">
</p>
...
## 2. Why a desktop will not do it | why a desktop refuses
...
## 3. Taking the connector, and what a lease actually grants | taking the connector
...
No trace was found of anybody else leasing a television this way, which is a
statement about a search rather than a claim of primacy.

## 4. The EDID we write ourselves | the EDID we write
...
**The block changes how the kernel classifies a connector, and nothing about
what is sent down it.** Every other decision in this project (the modeline,
the line rate, the refusal guard in section 10) is unaffected by it. Rewriting
a display's identity sounds more dangerous than it is.
...
Those bytes were worked out by trying them, because **there is no code in the
kernel that parses them.** The CEA extension block is handed to the display
microcontroller and the firmware hands back a version and two rates.[^6] There
was nothing in the tree to read, and `edid-decode` reading the result back as
"Vendor-Specific Data Block (AMD), Version 1.1" was the only confirmation
available.
...
## 5. What the protocol gives a compositor to work with | what the protocol gives
...
touch on the seat (it has a keyboard and nothing else), and Flyback
...
## 6. The scheduler, and why a variable rate has no deadline | the scheduler
...
## 7. A variable refresh rate, on a television from 1998 | a variable rate
...
That was the parameter set and unused. It has since been tested absent: taken
off the kernel command line, the machine rebooted, `/proc/cmdline` carrying no
`amdgpu` option at all. The compositor still reports the connector capable and
turns the variable rate on, and the scanout still follows. Asked for 58, 57,
56 and 55 Hz the television was given 58.06, 57.19, 55.99 and 55.01, and at 55
Hz sustained, over three hundred vblank intervals measured in a window opened
entirely after the change, every frame fell between 17.59 and 19.15 ms with
none at the mode's own 16.66. Every frame is stretched; nothing is dropped to
make an average. The kernel's own description of the parameter says what it is
for, and it is not this: *adds additional modes via VRR for refresh changes
without a full modeset*, which can be read on any machine with `modinfo -p
amdgpu`. Changing the refresh by a modeset is the thing this compositor does
not do.
...
**And a variable rate has a cost this project did not measure until the day
after publishing.** The owner of the television reported for two days that the
picture shimmers every so often, and once that it looked as though the format
had changed. Turning the variable rate off stops it and turning it back on
brings it back, twice each way.

The instrument could not see it, because the instrument was wrong. The gap
between two vblanks was timed with the compositor's own clock at the moment
its handler ran, which is when the process woke up rather than when the
hardware flipped. The driver's own timestamp for the same vblank was in hand
and was being given to clients, and was not used for this. The two quantities
are the same size, so a frame that was genuinely longer and a wakeup that was
merely late read identically, and both states reported the same spread.

On the driver's clock they are not alike at all. Three alternating windows of
twelve hundred frames each: with the rate fixed, not one frame in 3601
departed from the median by as much as twenty microseconds, which is a
metronome; with it variable, the bulk is just as steady and then one frame in
four hundred is stretched by up to eleven per cent. That is the same effect as
the beat above, which was measured at 4283 microseconds and seen plainly, at a
fifth of the size and a fraction of the time.

Turning it off is not the answer, because the cost and the benefit are one
mechanism seen from two sides. The frame ends when the flip lands, which is
what lets a client draw at the last moment, and also what stretches the frame
when the flip is late. Measured over three alternating windows, commit to the
start of scanout is 2.02 ms with the variable rate and 5.16 ms without it.

The knob that governs it is the slack a client is given on top of twice its own
drawing time, which is 1500 microseconds while the rate is variable. The
overruns that produce the worst frames are one to three milliseconds, so they
go straight through it. Widening it to 3000 costs 3.52 ms of latency, and to
5000 costs 5.51 ms, which is worse than giving the variable rate up: the whole
usable range is between 1500 and about 4500 microseconds.

**The threshold turns out to belong to the television, not to a percentage.**
A set's vertical countdown, once locked, works in a narrow window: a Philips
jungle datasheet gives 261 to 264 lines a field for the 60 Hz standard, and
says that a sync pulse falling outside it starts the retrace at the end of the
window rather than on the incoming sync. This mode's nominal is 262, so there
are two lines of room, 127 microseconds. Counted that way, the rate fixed left
the window not once in 3601 fields, and the variable rate left it three times
in 3600, about once every twenty seconds, which is the interval the owner
describes. The longest field measured 291.0 lines. The
rate walk of the day before put this set's cliff between 286 and 291: at 286
the picture is 0.2% short, inside that method's noise, and at 291 it is 15%
short. The worst field this compositor produces lands on the far side of a
cliff found a day earlier for an unrelated reason.

So the target is not "stretch it less". It is that no flip may be more than
127 microseconds late past the end of its frame, which slack can make rarer
and cannot make impossible.

What remains undecided is deliberately undecided. The overruns are the
compositor's own, they arrive in bursts, and the bursts follow the desktop
being used, because this television and one of the desktop's own outputs are
on the same graphics card. An idle machine does not produce them, so whether
3000 microseconds of slack removes what a person sees can only be found out
while somebody is using the machine. The numbers, and the three fixes
considered and rejected on the mechanism rather than on taste, are in
[`../data/vrr-frame-length-2026-09-13.txt`](../data/vrr-frame-length-2026-09-13.txt).

## 8. How any of this is measured | the instrument
...
variable rate off, which together are this compositor with its own scheduling removed), and
...
frame**, which is what any commit at a random phase waits for, whoever
...
the kernel's share of it; filmed at 240 fps the television shows nothing for
about 400 ms, because the converter and then the tube each add their own. That
is the cost the variable refresh rate avoids, and the first row is what says the
...
brochure. The largest absence first: **it does not race the beam**, which is
the one thing a leased connector makes possible on Linux and nobody has
built, here or anywhere else. The section ends with why.

No XWayland. No pointer, no touch, no input devices of any kind. No
...
## 11. Beside the others, and where that leaves it | beside the others
...
**Where Switchres is better than what is here**: it computes a modeline per
game from a monitor preset, across 15, 25 and 31 kHz and dozens
...
[^10]: The only prior report found is on multisync PC monitors rather than
       on televisions. Nothing was found for a consumer set, which says what
       the search turned up and not what exists.
